"""Shared offline Finding Hub import contract.

Canonical source: .github/graphify-review/scripts/import_contract.py.
The Hub vendors an identical copy; parity is regression-tested.
"""
...
class ValidationError(ValueError):
    """Invalid import contract; messages must not expose submitted values."""
...
@lru_cache
def review_validator(schema_directory):
    schemas = [
        json.loads((Path(schema_directory) / f).read_text(encoding="utf-8"))
...
def validate_envelope(data, idempotency_key, schema_directory):
    if len(canonical(data).encode("utf-8")) > 10 * 1024 * 1024:
        raise ValidationError("Import envelope exceeds 10 MiB.")
    ...
    if "clone_url" in repo and not isinstance(repo["clone_url"], str):
        raise ValidationError("Invalid clone URL.")
    ...
    error = next(review_validator(schema_directory).iter_errors(review), None)
    if error:
    ...
    if revision is not None and not isinstance(revision, str):
        raise ValidationError("Invalid authoritative source revision.")
