...
Distinguish required verification from optional Sonar feedback before checking. Document optional Sonar unavailability/partial coverage in `rationale`, not `checks` or `acceptance`; explicitly say the full Quality Gate was not verified. The manager also retains `sonar-assessment.md` and includes its limitations in the Hub summary. Never label missing analysis passed or hide required evidence: any Sonar obligation in the approved specification/repository policy, evidence needed for a criterion, or introduced regression still blocks `satisfied` when failed/unavailable. Every recorded check and acceptance criterion must pass for satisfaction; the validator has no optional-check bypass.
