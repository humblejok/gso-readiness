...
For Azure, shared `project.azure_api_version` defaults to `6.0`, compatible with Server 2020; `7.0` and `7.1` are explicit options for newer deployments. The user confirms `user.azure_devops_url` (the collection root, including any `/tfs` virtual path) during `/setup-review` or `/configure-review`. API authentication is refused until this approved destination exactly matches the detected collection. This prevents a changed remote from silently receiving Windows credentials or a PAT. A read-only repository GET pins its UUID; later operations reject a changed repository identity.
...
For PATs, select `pat` and run `azure_devops.py login` in the user's own interactive terminal after bootstrap. The native keyring binds the secret to the exact collection; no plaintext fallback exists. `logout` deletes only that local credential, not the server token. An approved secret manager may supply `GRAPHIFY_AZURE_PAT` together with a matching `GRAPHIFY_AZURE_PAT_URL`. PAT requests use the existing verified Python PEM/proxy transport, and the Hub token is never reused for Azure. Least-privilege Code read/write plus repository PR permissions are required for PR creation; read-only diagnostics do not prove write permissions.
