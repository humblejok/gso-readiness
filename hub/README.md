...
Requests start **Open**. Assign a project in the create/edit form to make a request eligible for `/analyse-requests` in that project's checkout. The command uses the selected model to inspect source, ask clarifying questions and publish specifications, moving the request to **Analyzed**. Users review/edit the analysis and explicitly **Accept analysis** to move it to **Specified**. Run `/implement-requests` to implement accepted specifications with isolated remote branches, native validation, optional Sonar MCP, direct independent verification and PRs. Confirmed completion marks **Implemented**, not merged/deployed. Manual implementation tracking remains available; **Closed** is a human action. Analysis/acceptance alone creates no code, branch/PR or Jira update.
...
Request data/history count toward workspace storage. Workspace exports and erasure include `change_requests`, `change_request_activities` and `request_implementations` (frozen approved specifications, claim outcomes, evidence and PR metadata). Analysis requires **requests:read** and **requests:analyse**; implementation requires **requests:read** and **requests:implement**, optionally restricted to one repository. Personal tech-lead tokens support these scopes with explicit workspace selection. Existing tokens are not silently upgraded. Apply migration **0010** before using implementation. Human acceptance/cancellation remains in the CSRF-protected web UI, not the analysis/implementation API. Changes to a claimed request invalidate the claim; four-hour claims and revision/digest checks reject stale workers. See [request analysis](../docs/request_analysis_workflow.md) and [implementation setup and recovery](../docs/request_implementation_workflow.md). Run PostgreSQL isolation/concurrency tests with a non-superuser, non-BYPASSRLS role before production deployment.
