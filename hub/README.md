...
Request data/history count toward workspace storage. Workspace exports include `change_requests` and `change_request_activities` with analysis snapshots; erasure includes both tables. The command requires **requests:read** and **requests:analyse** token scopes, optionally restricted to one repository. Personal tech-lead tokens support these scopes with explicit workspace selection. Existing tokens are not silently upgraded. Human acceptance/cancellation remains in the CSRF-protected web UI, not the analysis API. See [request analysis setup and contract](../docs/request_analysis_workflow.md). Run PostgreSQL isolation/concurrency tests with a non-superuser, non-BYPASSRLS role before production deployment.
