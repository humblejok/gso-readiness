...
The implementation manager now invokes **Targeted Finding Verifier directly** and prepares/packages its returned evidence itself. No nested subagents setting is required. A readiness-only call checks that the verifier is callable before acquiring a new claim; actual verification still runs independently before and after committing. Update the project's `.github` kit and start a fresh chat so the new agent allowlist is loaded. Standalone `/revalidate-finding` is unchanged. See [agent availability and recovery](docs/finding_implementation_workflow.md#agent-availability-and-recovery).
